...
class Word2Vec(utils.SaveLoad):
    ...
    def __init__(self,  sentences=None, size=100, alpha=0.025, window=5, min_count=5,
        sample=0, seed=1, workers=1, min_alpha=0.0001, sg=1, hs=1, negative=0, cbow_mean=0,
                 word_number=0, topic_number=0, topic_size=100):
        """
        ...
        self.layer_topic_size = int(topic_size)
        self.layer2_size = self.layer1_size + self.layer_topic_size
        if size % 4 != 0:
    ...
    def build_vocab(self, sentences):
        ...
        for sentence_no, sentence in enumerate(sentences):
            ...
            for word in sentence:
                total_words += 1
                if word[2] in vocab:
                    vocab[word[2]].count += 1
                else:
                    vocab[word[2]] = Vocab(count=1)
    ...
    def train(self, sentences, total_words=None, word_count=0, chunksize=100):
        ...
        def prepare_sentences():
            for sentence in sentences:
                ...
                sampled = [(self.vocab[word[2]], word[0], word[1]) for word in sentence if word[2] in self.vocab and (self.vocab[word[2]].sample_probability >= 1.0 or self.vocab[word[2]].sample_probability >= random.random_sample())]
                yield sampled
    ...
    def reset_weights(self,sentences):
        ...
        if self.hs:
            self.syn1 = zeros((len(self.vocab), self.layer2_size), dtype=REAL)
        if self.negative:
            self.syn1neg = zeros((len(self.vocab), self.layer2_size), dtype=REAL)
        #self.syn0norm = None
        #topic and the word number
        #get the topic number
        #sentence_word = [item[0] for item in sentences]
        #sentence_topic = [item[1] for item in sentences]
    
        #self.word_number = len(set(sentence_word))
        #self.topic_number = len(set(sentence_topic))
        #self.word_number = 0
        #self.topic_number = 0
        #for sentence in sentences:
        #    for item in sentence:
        #        if item[0] > self.word_number:
        #            self.word_number = item[0]
        #        if item[1] > self.topic_number:
        #            self.topic_number = item[1]
        #self.topic_number +=1
        #self.word_number +=1


        self.syn0 = empty((self.word_number, self.layer1_size), dtype=REAL)
        for i in xrange(self.word_number):
            self.syn0[i] = (random.rand(self.layer1_size) - 0.5) / self.layer1_size
        self.syn0_topic = empty((self.topic_number, self.layer_topic_size), dtype=REAL)
        for  i in xrange(self.topic_number):
            self.syn0_topic[i] = (random.rand(self.layer_topic_size)-0.5)/self.layer_topic_size
